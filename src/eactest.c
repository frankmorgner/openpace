...
static const struct ef_cardaccess ef_cardsecurity_files[] = {
    {
        ef_cardsecurity_wolf,
        sizeof ef_cardsecurity_wolf,
    },
};
...
static int
test_ef_cardsecurity_parsing(const struct ef_cardaccess tc)
{
    int failed = 1;
    BUF_MEM *pubkey = NULL;
...
test_parsing(void)
{
    ...
    for (i = 0; i<(sizeof ef_cardsecurity_files)/sizeof(*ef_cardsecurity_files); i++) {
        failed += test_ef_cardsecurity_parsing(ef_cardsecurity_files[i]);
    }
    TESTEND;
